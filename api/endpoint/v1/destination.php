    ...
    function outputManyEvent(){
        ...
        if($_GET['type'] === "next"){
            if($this->totalRecord > $_GET['limit'])
                $next_url = $this->app->getConfig('apipath').$this->app->url(null,array_merge(['limit'=>$_GET['limit'],'offset'=>$next_offset,'type'=>"next",'city'=>$_GET['city']],$param_array));
            if(isset($_GET['offset']) and ($_GET['offset'] > 0) )
                $previous_url = $this->app->getConfig('apipath').$this->app->url(null,array_merge(['limit'=>$_GET['limit'],'offset'=>$previous_offset,'type'=>"previous",'city'=>$_GET['city']],$param_array));
        }
