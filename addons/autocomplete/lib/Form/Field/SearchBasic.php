...
class Form_Field_SearchBasic extends \Form_Field_Hidden
{
    ...
    function init()
    {
        ...
        // $this->api->pathfinder->base_location->base_path = str_replace("frontend", , subject)
    ...
    function addCondition($q)
    {
        ...
        */
        $this->app->memorize('search_term',$q);

        $this->model->addExpression('Relevance')->set('MATCH(search_string) AGAINST ("'.trim(implode(',', explode(" ",$q)),",").'" IN NATURAL LANGUAGE MODE)');
        $this->model->addCondition(
                        $this->model->dsql()->orExpr()
                        // ->where($this->model->getElement('Relevance'),'>',0)
                        ->where($this->model->getElement('search_string'), 'like', '%'.$q.'%')
                        );
        $this->model->setOrder('Relevance','Desc');

        $selected_city_id = $this->app->recall('form_city_id');
        if($selected_city_id > 0){
            $this->model->addCondition('city_id',$selected_city_id);
        }

        $selected_venue_id = $this->app->recall('form_venue_id');
        if($selected_venue_id > 0){
            $this->model->addCondition('venue_id',$selected_venue_id);
        }
