...
class Controller_Captcha extends \Controller {
    ...
    private function addCaptcha() {
        if ($_GET['captcha_view']) {
            ...
            $view->setHTML('<img style="cursor:pointer" src="'.$this->api->url(null,array('captcha_view'=>'true','rand'=>md5(microtime()))).'" /><i style="font-size:20px;" class="icon-cw atk-effect-warning atk-bold">Refresh</i>');
