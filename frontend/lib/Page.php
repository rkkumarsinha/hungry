...
class Page extends AbstractView {
    ...
    function addMetaTag(){        
        // echo $this->app->page."";
        // set meta tags
        $page_name = $this->app->page;
        $page_meta = $this->add('Model_SeoConfig');
        $page_meta->addCondition('page_name',$page_name);
        $page_meta->tryLoadAny();
        if($page_meta->loaded()){

            $this->setMetaTag('keyword',$page_meta['keyword']);
            $this->setMetaTag('description',$page_meta['description']);
            $this->setTitle($page_meta['title']);

            $meta_tag = $this->add('Model_MetaTag');
            $meta_tag->addCondition('seo_config_id',$page_meta->id);
            foreach ($meta_tag as $meta) {
                $this->setMetaTag($meta['meta_property'],$meta['meta_content']);
            }
